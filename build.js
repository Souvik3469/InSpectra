...
import {
  cpSync, readFileSync, writeFileSync,
...
console.log(`\n🔨 Building QuickConsole${isWatch ? ' (watch mode)' : ''}...\n`)
...
function asciiEscape(filePath) {
  ...
  const escaped = raw.replace(/[^\x00-\x7F]/gu, (ch) =>
...
function postProcess() {
  ...
  if (isWatch) {
    writeFileSync(resolve(distDir, 'version.txt'), Date.now().toString(), 'utf-8')
  }
}
...
if (!isWatch) {
  ...
  console.log('🧩 chrome://extensions  →  Developer mode  →  Load unpacked → dist/\n')
}
